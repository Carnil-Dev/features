    ...
    return {
      ...
      createdAt:
        events.length > 0
          ...
          : new Date(),
      updatedAt:
        events.length > 0 ? events[0]?.timestamp || new Date() : new Date(),
    };
  }
    ...
    if (reportType === "gdpr") {
      ...
      if (dataAccessEvents.length > 0) {
        findings.push({
          ...
          recommendation:
            "Ensure all data access is properly logged and authorized",
          status: "open",
          createdAt: new Date(),
        });
      }
